...
import { setTvShows } from '../Slice/MovieSlice'
...
export const TvShows = () => {
    ...
    const FetchData = async () => {
        setIsLoading(true)
        const response = await fetchApi(`https://api.themoviedb.org/3/discover/tv`,
            {
                headers: {
    ...
    async function sortByFilter() {
        ...
        const data = await fetchApi(`https://api.themoviedb.org/3/discover/tv?sort_by=${selectedBySortOption.value}`,
            {
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${import.meta.env.VITE_APP_TOKEN}`,
